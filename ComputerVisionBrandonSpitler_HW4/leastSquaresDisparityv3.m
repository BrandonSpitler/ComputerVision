function [ disparityMatrix ] = leastSquaresDisparityv3(  e1,e2,maxDisp,occ ,windowSize )
        ...
        for j=1:1:c1
          for i=j:1:min(c1,j+maxDisp)
               if(j==1)
                   ...
                   [minScore,dir]=max([dynamicArray(i-1,j-1,1),dynamicArray(i,j-1,1)-occ,dynamicArray(i-1,j,1)-occ]);
                   
               end
               e1X=e1(row:row+windowSize*2,i:i+windowSize*2);
               e2X=e2(row:row+windowSize*2,j:j+windowSize*2);
               correlCoef=corr(e1X(:),e2X(:))^2;
               if (i==c1)
                  dynamicArray(i,j,1)=correlCoef-(c1-j)*occ+minScore;
                  dynamicArray(i,j,2)=dir;
               elseif (j==c1)
                  dynamicArray(i,j,1)=correlCoef-(c1-i)*occ+minScore;
                  dynamicArray(i,j,2)=dir;
               else
                  dynamicArray(i,j,1)=correlCoef+minScore ;
                  dynamicArray(i,j,2)=dir;
               end
