function [ disparityMatrix ] = NCCSDisparityv2( e1,e2,maxDisp,occ ,windowSize )
        ...
        for j=1+windowSize:windowSize*2+1:c1
          ...
          for i=j:windowSize*2+1:min(c1,j+maxDisp)
               if(j==1+windowSize)
                   ...
                   [minScore,dir]=min([dynamicArray(ceil((i-1-windowSize)/(2*windowSize+1)),ceil((j-1-windowSize)/(2*windowSize+1)),1),...
               ...
               nccsVar=sum(sum((double(e1X)-double(e2X)).^2));
               if (i==c1)
